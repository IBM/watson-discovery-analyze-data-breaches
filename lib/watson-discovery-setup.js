        ...
        for (let i = 0, size = environments.length; i < size; i++) {
          ...
          if (validateID) {
            if (validateID === environment.environment_id) {
              console.log('Found Discovery environment using DISCOVERY_ENVIRONMENT_ID: ' + environment.environment_id + ' [name:' +  environment.name + ']');
              params.environment_id = environment.environment_id;
              return resolve(params);
            }
          } else {
            if (environment.name === DISCOVERY_ENVIRONMENT_NAME) {
              console.log('Found Discovery environment by name: ' + environment.name + ' [id:' + environment.environment_id + ']');
              params.environment_id = environment.environment_id;
              return resolve(params);
...
WatsonDiscoverySetup.prototype.createDiscoveryCollection = function(params) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const createCollectionParams = {
      ...
      description: 'Discovery collection created by watson-discovery-analyze-data-breaches.',
      language_code: 'en_us'
    };
    ...
    this.discoveryClient.getCollection(params, (err, data) => {
      if (err) {
        ...
        if ((! params.checkedForExistingDocs) && (numDocsLoaded > 0)) {
          ...
          params.checkedForExistingDocs = true;
          var loadedDocs = params.loadedDocs;
          var totalDocsToProcess = loadedDocs + docCount;
          ...
          for (let i = 0; i < docCount; i++) {
            ...
            this.discoveryClient.addDocument(addDocParams, (err, data) => {
              if (err) {
                ...
                console.log('[' + params.loadedDocs + '] Added document: ' + data.document_id);
                params.loadedDocs += 1;
                if (params.loadedDocs === totalDocsToProcess) {
                  this.discoveryClient.getCollection(params, (err, data) => {
                    if (err) {
                      console.error(err);
                    } else {
...
/**
 * Add files to Discovery collection.
 */
